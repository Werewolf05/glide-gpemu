...
<section id="page-scheduler" class="page"><h1 class="page-heading">Scheduler</h1><div class="review-cards"><div class="review-card"><h3>FIFO</h3><p>Requests execute in arrival order.</p><span class="badge">BASELINE</span></div><div class="review-card"><h3>SJF</h3><p>Shortest profiled compute time first.</p><span class="badge blue">BASELINE</span></div><div class="review-card"><h3>HASP</h3><p>Affinity scheduling with aging boost.</p><div class="formula">score = (1/compute_ms) × memory_fit × (1 + age/threshold)</div><span class="badge yellow">NOVEL CONTRIBUTION</span></div></div><div class="review-card"><h3>Real scheduler comparison</h3><div id="schedulerLoading" class="muted">Computing results from real profiled data...</div><table class="review-table"><thead><tr><th>Metric</th><th>FIFO</th><th>SJF</th><th>HASP</th></tr></thead><tbody id="schedulerRows"></tbody></table></div><div class="review-grid" style="margin-top:12px"><div class="review-card"><h3>Latency and fairness</h3><canvas id="schedulerChart" height="150"></canvas></div><div class="review-card"><h3>HASP decision explanation</h3><div id="haspDecision" class="muted">Waiting for scheduler data...</div></div></div></section>
...
<section id="page-experiments" class="page"><h1 class="page-heading">Experiment Results</h1><div id="experimentLoading" class="muted">No experiment data yet.</div><button id="runExperiments" class="action" style="margin:10px 0">Run all experiments (~60 seconds)</button><div id="experimentProgress" class="muted"></div><div class="review-grid"><div class="review-card"><h3>Scenario comparison</h3><canvas id="scenarioChart" height="180"></canvas></div><div class="review-card"><h3>What to look for</h3><p class="muted">Lower latency and starvation are better. Higher fairness and throughput are better. Error bars appear when repeated reports contain confidence intervals.</p></div></div><div id="experimentResults"></div></section>
</main>
<script>
const $=id=>document.getElementById(id), colors={orange:'#ff9900',blue:'#5794f2',green:'#73bf69',red:'#f2495c',yellow:'#fade2a',gray:'#737b84',grid:'#2c3235'};
let computeChart,latencyChart,lastMetrics=null;
let engineReplayTimer=null;
document.querySelectorAll('[data-page="about"],#page-about').forEach(element=>element.remove());
['engine','scheduler','profiler','experiments'].forEach(name=>{
...
async function loadScheduler(){const data=await (await fetch('/api/scheduler_comparison',{cache:'no-store'})).json();$('schedulerLoading').style.display='none';const rows=[['Avg latency','avg_latency_ms','ms'],['P95 latency','p95_latency_ms','ms'],['Throughput','throughput','/s'],['Fairness index','jains_fairness_index',''],['Starvation','starvation_count','']];$('schedulerRows').innerHTML=rows.map(row=>`<tr><td>${row[0]}</td>${['fifo','sjf','hasp'].map(name=>`<td style="${name==='hasp'?'color:var(--orange)':''}">${num(data[name]?.[row[1]]).toFixed(row[1]==='jains_fairness_index'?3:2)}${row[2]}</td>`).join('')}</tr>`).join('');renderSchedulerCharts(data);const decision=data.hasp?.decision_trace?.[0];const target=$('haspDecision');if(decision&&target)target.innerHTML=`<table class="review-table"><tr><th>Factor</th><th>Value</th></tr><tr><td>Selected request</td><td>${decision.request_id} · ${decision.model}</td></tr><tr><td>Compute time</td><td>${num(decision.compute_ms).toFixed(2)} ms</td></tr><tr><td>Memory usage</td><td>${num(decision.memory_mb).toFixed(2)} MB</td></tr><tr><td>Wait time</td><td>${num(decision.wait_seconds).toFixed(3)} s</td></tr><tr><td>Compute affinity</td><td>${num(decision.compute_affinity).toFixed(5)}</td></tr><tr><td>Memory-fit score</td><td>${num(decision.memory_fit_score).toFixed(2)}</td></tr><tr><td>Aging boost</td><td>${num(decision.age_boost).toFixed(3)}</td></tr><tr><td>Final HASP score</td><td><b>${num(decision.final_score).toFixed(5)}</b></td></tr></table>`;else if(target)target.textContent='No HASP decision data returned.'}
function renderSchedulerCharts(data){const canvas=$('schedulerChart');if(!canvas||typeof Chart==='undefined')return;if(window.schedulerChart&&typeof window.schedulerChart.destroy==='function')window.schedulerChart.destroy();window.schedulerChart=new Chart(canvas,{type:'bar',data:{labels:['Avg latency (ms)','P95 latency (ms)','Fairness'],datasets:['fifo','sjf','hasp'].map((name,index)=>({label:name.toUpperCase(),data:[num(data[name]?.avg_latency_ms),num(data[name]?.p95_latency_ms),num(data[name]?.jains_fairness_index)*100],backgroundColor:[colors.gray,colors.blue,colors.orange][index]}))},options:{...chartOptions(),plugins:{legend:{display:true}}}})}
  ...
  try:
    for scheduler_name in ('fifo', 'sjf', 'hasp'):
      ...
      results[scheduler_name] = {
        ...
        'decision_trace': engine.scheduler.get_stats().get('decision_trace', []),
        'completion_times_s': [
...
def _run_experiments_background() -> None:
  ...
  try:
    ...
    try:
      ...
      from glide.report import build_report, export_report
    export_report(build_report(results, {
      'gpu': 'Tesla_V100-PCIE-32GB',
      'models': ['resnet18', 'resnet50', 'alexnet'],
      'schedulers': list(SCHEDULER_NAMES),
      'seed': 42,
      'duration_s': 15.0,
    }), os.path.join(GLIDE_DIR, 'experiment_reports'))
...
def _start_experiments() -> bool:
  ...
  with _EXPERIMENT_LOCK:
    ...
    try:
      ...
      print(f'[GLIDE experiments] unable to clear stale cache: {exc}')
...
@app.route('/api/model_decomposition')
def api_model_decomposition():
  model_name = _normalize_model(request.args.get('model', _load_selected_model()))
  gpu_name = _normalize_gpu(request.args.get('gpu', _load_selected_gpu()))
  try:
    import torchvision.models as models
    try:
      from .model_analyser import decompose_model
    except ImportError:
      from glide.model_analyser import decompose_model
    model = getattr(models, model_name)(weights=None)
    return jsonify(decompose_model(model, gpu_name, model_name))
  except (AttributeError, RuntimeError, ModuleNotFoundError) as exc:
    return jsonify({'error': str(exc), 'model': model_name, 'gpu': gpu_name}), 400
